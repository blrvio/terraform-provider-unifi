...
description: |-
  ...
  This resource is intentionally narrow: it manages the isolation-related fields plus jumboframe_enabled and flowctrl_enabled of the controller's global_switch setting object. All other fields of that object (such as DHCP snooping, 802.1X and STP) are preserved untouched using a read-modify-write write path, so this resource can be adopted without clobbering settings managed elsewhere (for example, DHCP snooping via unifi_setting_usw).
...
- `flowctrl_enabled` (Boolean) Enable IEEE 802.3x flow control site-wide on managed switches. Usually left disabled on modern networks (can induce head-of-line blocking). Optional and Computed: if unset, the current controller value is retained.
- `jumboframe_enabled` (Boolean) Enable Jumbo Frames (MTU up to 9216 bytes) site-wide on managed switches. Improves throughput/efficiency for large-frame workloads (e.g. NAS/SAN, backups, SIEM). All devices in the path must support jumbo frames. Optional and Computed: if unset, the current controller value is retained.
