...
The provider authenticates exclusively with an **API Key** (requires controller version 9.0.108 or later).

~> **Breaking change:** username/password authentication was removed in the go-unifi v10 based release. The `username`/`password` attributes (and `UNIFI_USERNAME`/`UNIFI_PASSWORD`) are deprecated and now raise a configuration error — migrate to `api_key` (or `UNIFI_API_KEY`).
...
- `api_key` (String, Sensitive) API Key for the user accessing the API. Can be specified with the `UNIFI_API_KEY` environment variable. This is now the only supported authentication method; controller version 9.0.108 or later is required.
...
- `password` (String, Sensitive, Deprecated) **Deprecated and no longer supported.** The provider now authenticates exclusively with an API key; username/password authentication was removed in go-unifi v10. Set `api_key` (or `UNIFI_API_KEY`) instead. Username/password authentication was removed in go-unifi v10. Use `api_key` instead.
...
- `username` (String, Deprecated) **Deprecated and no longer supported.** The provider now authenticates exclusively with an API key; username/password authentication was removed in go-unifi v10. Set `api_key` (or `UNIFI_API_KEY`) instead. Username/password authentication was removed in go-unifi v10. Use `api_key` instead.
...
| Authentication | Username/Password | API Key (username/password removed in v10) |
...
2. **Authentication Updates**
   ...
   Authentication is now **API Key only** (controller v9.0.108+). Username/password authentication
   was removed in the go-unifi v10 based release — replace it with an API key:
